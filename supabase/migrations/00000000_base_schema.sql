-- Vteam 기본 스키마 (베이스 마이그레이션)
-- supabase/schema.sql 의 내용을 마이그레이션 체계에 편입한 것.
-- 이후 20260528_* 등 기능 마이그레이션이 이 위에 컬럼/테이블을 덧붙인다.
-- 이미 테이블이 존재하는 DB(대시보드로 먼저 구성한 라이브 DB)에 돌려도
-- 안전하도록 모든 테이블은 `create table if not exists` 로 작성한다.
...
create table if not exists profiles (
  ...
  avatar_url text,
  position text,
  reports_to uuid references profiles(id),ㅃ
...
-- 회사 근무 규정
create table if not exists company_work_settings (
  id uuid primary key default gen_random_uuid(),
